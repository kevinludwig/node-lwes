...
var esfparser = require('./esf-parser'),
...
function checkFields(template, ev) {
    return Object.keys(ev.attributes).map(function(key) {
        ...
        if (val.nullable && !isarray) return "nullable not valid on non-array type, "+key;
        if (types.nameToId(val.typeName, isarray, val.nullable) !== attrib.type) 
            return "type mismatch for "+key;
