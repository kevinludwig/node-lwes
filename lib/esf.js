...
function onArrayType(token,type) {
    if (state === ARRAY_START) {
        ...
        else throw new Error("ESF: expecting integer-value, got "+type);
    }
...
function onDefaultValue(token,type) {
    var declaredType = db[eventName][currentKey].type;
    if (['int16','uint16','int32','uint32','int64','uint64'].indexOf(declaredType) !== -1) {
        if (type !== 'integer-value') throw new Error("ESF: expecting default integer-value");
        else db[eventName][currentKey].value = +token;
    } 
    else if (['float','double'].indexOf(declaredType) !== -1) {
        if (type !== 'decimal-value' && type !== 'integer-value') throw new Error("ESF: expecting default decimal-value, got:"+type);
        else db[eventName][currentKey].value = parseFloat(token);
    }
    else if (declaredType === 'string') {
        if (type !== 'quoted-string') throw new Error("ESF: expecting default quoted-string");
        else db[eventName][currentKey].value = token.replace(/^\"/,"").replace(/\"$/,"");
    }
    else throw new Error("ESF: unsupported default type");
    state = END_ATTRIBUTE;
}
...
module.exports = function(filename, callback) {
    ...
    t.addRule(/^\d+\.\d+$/, "decimal-value");
    t.addRule(/^\d+$/, "integer-value");
    t.addRule(/^true|false$/, "boolean-value");
